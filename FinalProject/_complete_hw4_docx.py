...
DOC_PATH = Path(
    r"C:\Users\emmak\Desktop\CodingProjects\DatabaseClass\CS 486_586 – Homework 4-emmanart.docx"
)
...
def main():
    ...
    spy7 = [
        "",
        "Answer 7:",
        "SQL (subquery): Agents on a mission are on that mission's team (teamrel links agent to team; mission has team_id).",
        ...
        "    SELECT m.team_id",
        "    FROM mission m",
        "    WHERE m.access_id IN (",
        "      SELECT sc_id",
        "      FROM securityclearance",
        "      WHERE sc_level = 'Top Secret'",
        "    )",
        ...
        "",
        "Note: If your data uses a different status spelling, adjust the failed filter (for example 'Failed').",
        "",
    ]
    insertions.append((34, spy7, [(4, 19)]))
    ...
    spy8 = [
        ...
        "FROM language l",
        "INNER JOIN languagerel lr ON l.lang_id = lr.lang_id",
        ...
        "[SCREENSHOT: first 5 rows — or all rows if fewer than 5]",
        "",
        "Explanation: Count agents per language, then keep only language(s) whose count equals the maximum.",
        "",
    ]
